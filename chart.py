...
#logs/에 속한 폴더 중에서 가장 최근에 생성된 폴더명을 반환
def latest_log_folder():
    if "logs" not in os.listdir("."):
        print("no 'logs' folder in current directory")
        sys.exit()

    log_folders = os.listdir("./logs")
    ...
    if log_folders:
        ...
        latest_log=None
        print("Nothing saved in 'logs' folder")
        return None

# --- 설정 ---
MAIN_TITLE = "Blockcopy Performance (per copy)"  # Chart Title
LOG_DIR = "./logs/" + latest_log_folder() #baseline과 RPC 로그가  위치한 folder
