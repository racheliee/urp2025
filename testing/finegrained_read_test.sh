...
for bin in $HOME_DIR/create_file $HOME_DIR/client_read $HOME_DIR/baseline_read ./compare; do
    ...
    for fs in "${file_sizes[@]}"; do
        for bc in "${block_copies[@]}"; do
            for bn in "${byte_sizes[@]}"; do
                ...
                # (2) client: a.txt
                echo "[client] sudo $HOME_DIR/client_read 10.0.0.2 $a -n $it -b $bn -s $seed"
                sudo $HOME_DIR/client_read 10.0.0.2 "$a" -n "$it" -b "$bn" -s "$seed" -t \
                    > >(stdbuf -oL tee -a "$RPC_LOG_FILE" >/dev/null) \
                    2> >(stdbuf -eL tee -a "$RPC_LOG_FILE" >&2)
                flush_caches

                # (3) baseline: b.txt
                echo "[baseline] sudo $HOME_DIR/baseline_read $b -n $it -b $bn -s $seed"
                sudo $HOME_DIR/baseline_read "$b" -n "$it" -b "$bn" -s "$seed" -t \
