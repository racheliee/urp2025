...
def latest_log_folder():
    if "random_logs" not in os.listdir("../testing"):
        print("no 'random_logs' folder in testing directory")
        sys.exit()

    log_folders = os.listdir("../testing/random_logs")
    ...
    if log_folders:
        ...
        latest_log=None
        print("Nothing saved in 'random_logs' folder")
        return None

# --- 설정 ---
MAIN_TITLE = "Performance (per copy)"
LOG_DIR = "../testing/random_logs/" + latest_log_folder() #최신 baseline과 RPC 로그가  위치한 folder
...
# 스택 컬럼 순서
PARTS = ["read_s", "write_s", "io_s", "fiemap_s", "rpc_s", "other_s"]
...
to_numeric(baseline, ["block_num","iterations","total_blocks","file_size_gb","total_s"])
to_numeric(rpc,      ["block_num","iterations","total_blocks","file_size_gb","total_s"])

# RPC → 공통 이름으로 리매핑
rpc.rename(columns={
    "server_read_s": "read_s",
    "server_write_s": "write_s",
    "server_other_s": "other_s",
}, inplace=True)

# 누락 가능 컬럼들 생성(0으로)
maybe_cols = ["read_s","write_s","io_s","fiemap_s","rpc_s","other_s", "prep_s", 'end_s']
baseline = ensure_columns(baseline, maybe_cols)
rpc      = ensure_columns(rpc,      maybe_cols)

# 단위 보정 (합치기 전에 각 성분을 초로 맞춤)
ns_to_s_if_needed_df(baseline, maybe_cols + ["total_s"])
ns_to_s_if_needed_df(rpc,      maybe_cols + ["total_s"])

# other_ns <- other_ns + prep_ns + end_nbaseline["other_s"] = baseline[["other_s","prep_s","end_s"]].sum(axis=1, skipna=True)
rpc["other_s"]      = rpc[["other_s","prep_s","end_s"]].sum(axis=1, skipna=True)
...
for df in (baseline, rpc):
    for c in ["block_num","iterations","total_blocks","file_size_gb"]:
        df[c] = pd.to_numeric(df[c], errors="coerce")

# ------ 그룹핑 ------
group_keys = ["file_size_gb","iterations"]
...
for (fs, nbc) in common_groups:
    ...
    b_per_iter = b_g.groupby(["block_num","iterations"], as_index=False)[PARTS].mean().fillna(0.0)
    r_per_iter = r_g.groupby(["block_num","iterations"], as_index=False)[PARTS].mean().fillna(0.0)
    ...
    def plot_stack(xs, stacks, width, parts):
        ...
        for vals, part in zip(stacks, parts):
            ...
            for xi, v, bot in zip(xs, vals, bottoms):
                if v > 0 and part in ['read_s', 'write_s']: #지분 큰 요소만
                    ax.text(
                        xi, bot + v/2.0,
    ...
    # 축 / 서브타이틀 / 격자
    ax.set_title(f"file_size={fs} GiB · iterations={nbc}", fontsize=11)
    ...
    out_path = os.path.join(OUT_DIR, f"chart_fs{safe_fs}GB_copies{safe_nbc}-random-{str(datetime.now().isoformat())[:10]}.png")
