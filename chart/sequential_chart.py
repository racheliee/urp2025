...
def latest_log_folder():
    if "sequential_logs" not in os.listdir("../testing"):
        print("no 'sequential_logs' folder in testing directory")
        sys.exit()

    log_folders = os.listdir("../testing/sequential_logs")
    ...
    if log_folders:
        ...
        latest_log=None
        print("Nothing saved in 'sequential_logs' folder")
        return None

# --- 설정 ---
MAIN_TITLE = "Performance (per copy)"
LOG_DIR = "../testing/sequential_logs/" + latest_log_folder() #최신 baseline과 RPC 로그가  위치한 folder
...
for (fs, nbc) in common_groups:
    ...
    # 축 / 서브타이틀 / 격자
    ax.set_title(f"file_size={fs} GiB · num_block_copies={nbc}", fontsize=11)
    ...
    out_path = os.path.join(OUT_DIR, f"chart_fs{safe_fs}GB_nbc{safe_nbc}-{str(datetime.now().isoformat())[:10]}.png")
