...
def latest_log_folder():
    if "finegrained_write_logs" not in os.listdir("../testing"):
        print("no 'finegrained_write_logs' folder in testing directory")
        sys.exit()

    log_folders = os.listdir("../testing/finegrained_write_logs")
    ...
    if log_folders:
        ...
        latest_log=None
        print("Nothing saved in 'finegrained_write_logs' folder")
        return None

# --- 설정 ---
MAIN_TITLE = "Finegrained Write Performance (per copy)"  # ← 여기서 제목을 직접 정하세요
LOG_DIR = "../testing/finegrained_write_logs/" + latest_log_folder() #baseline과 RPC 로그가  위치한 folder
...
for (fs, nbc) in common_groups:
    ...
    out_path = os.path.join(OUT_DIR, f"chart_fs{safe_fs}GB_bytes{safe_nbc}-finegrained-write-{str(datetime.now().isoformat())[:10]}.png")
