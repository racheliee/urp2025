...
static void usage(const char *prog) {
    fprintf(stderr,
        "Usage: %s <server_eternity> <file_path> [-b block_size] [-n iterations] [-s seed] [-p] [-l] [-t]\n"
        "Options:\n"
...
int main(int argc, char *argv[]) {
    ...
    int opt;
    while ((opt = getopt(argc, argv, "b:n:s:plt")) != -1) {
        switch (opt) {
        case 'b': {
    ...
    for (long i = 0; i < iterations; i++) {
        if(log) {
            if(i % 1000 == 0) {
                ...
                fprintf(stderr, "\rFinegrained Read Baseline Test: %ld / %ld (%6.1f%% ) | %6.2fs", i, iterations, (double) i / iterations * 100, elapsed);
            }
        }
        ...
        if(print) {
            printf("%ld\t%.*s\n", i, (int)bytes_size, expected_buf + offset_in_block);
        }
        free(expected_buf);
    ...
    if(log) {
        ...
        fprintf(stderr, "\rFinegrained Read Baseline Test: %ld / %ld (%6.1f%% ) | %6.2fs", iterations, iterations, (double) 100, elapsed);
    }
    ...
    if(csv) {
        // byte_num, iteration, # of bytes read, file_size, Read time, Write time, I/O time, Total time
    ...
    printf("\n\n");
    printf("------------ Finegrained Read Baseline Test Results ------------\n");
